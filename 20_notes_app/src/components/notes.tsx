...
export default function NotesApp() {
  ...
  return (
    <div className="flex flex-col h-screen bg-background bg-slate-200  text-foreground">
      <header className="bg-muted p-4 shadow">
        <h1 className="text-2xl text-blue-500 font-bold">📝Note Taker</h1>
      </header>
      <main className="flex-1 overflow-auto p-4">
        <div className="mb-4">
          ...
          {editingNoteId === null ? (
            <Button onClick={handleAddNote} className="mt-2 bg-yellow-300 hover:bg-blue-800 ">
              Add Note
            </Button>
          ) : (
            <Button onClick={handleUpdateNote} className="mt-2 bg-sky-400 hover:bg-blue-800">
              Update Note
            </Button>
          )}
        </div>
        ...
        <div className="grid gap-4">
          {notes.map((note) => (
            <Card key={note.id} className="p-4">
              <div className="flex items-center text-pink-600 justify-between">
