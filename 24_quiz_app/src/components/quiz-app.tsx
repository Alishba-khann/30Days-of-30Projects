...
export default function QuizApp() {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-blue-200 text-foreground">
      {state.showResults ? (
        // Show results if the quiz is finished
        <div className="bg-card p-8 rounded-lg shadow-md w-full max-w-md">
          <h2 className="text-2xl text-blue-700 font-bold mb-4">Results</h2>
          <p className="text-lg mb-4">
            You scored {state.score} out of {state.questions.length}
          </p>
          <Button onClick={resetQuiz} className="w-full bg-blue-400 hover:to-blue-800">
            Try Again
          </Button>
        </div>
      ) : (
