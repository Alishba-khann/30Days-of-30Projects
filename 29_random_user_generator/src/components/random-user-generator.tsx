...
const RandomUser: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-white">
      ...
      <Button onClick={fetchRandomUser} className="mb-6 bg-teal-900 hover:bg-teal-950">
        Fetch New User
      </Button>
      {loading && (
      ...
      {user && (
        <Card className="border-0 shadow-lg rounded-lg overflow-hidden max-w-sm relative">
          <CardHeader className="h-32 bg-[#253d36] relative">
            <Image 
              src={user.image}
              alt={user.name}
              width={80}
              height={80}
              className="rounded-full border-4  border-white font-serif absolute bottom-0 left-1/2 transform -translate-x-1/2 translate-y-1/2"
            />
          </CardHeader>
