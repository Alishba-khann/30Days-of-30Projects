...
export default function RecipeSearch() {
  ...
  const handleSearch = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://api.edamam.com/search?q=${query}&app_id=97a7d439&app_key=8c88d04a5c9cc307f970f163d58f0a15`
      );
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-6xl bg-yellow-100 mx-auto p-4 md:p-6">
      {/* Header section */}
      <header className="flex flex-col items-center mb-6">
        <h1 className="text-3xl font-bold text-blue-700 mb-2">Recipe Search</h1>
        ...
        <div className="mb-4 text-blue-700">
          ...
          <div className="flex space-x-2">
            {examples.map((examples) => (
              <span
                key={examples}
                className="px-2 py-1 bg-yellow-200 rounded-md cursor-pointer"
                onClick={() => setQuery(examples)}
              >
                {examples}
              </span>
            ))}
        ...
        <form className="relative w-full max-w-md mb-6" onSubmit={handleSearch}>
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            className="absolute right-2  hover:bg-yellow-200 top-1/2 -translate-y-1/2"
          >
